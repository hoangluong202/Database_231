import { NOT_FOUND_GENERIC, PrismaErrCode, TRY_LATER } from '@be/constants';
...
export function customErrorHandler(err: FastifyError, _req: FastifyRequest, res: FastifyReply) {
    ...
    if (validation.keyword === 'required') {
        // @ts-expect-error: We're assuming that validation.params.missingProperty is always defined here.
